...
test('android package and msal setup target io.ifwlzs.jumusic.lx debug and release registrations', () => {
  ...
  const releaseAuthConfigTemplate = readFile('android/app/msal/auth_config_single_account.template.json')
  const releaseWorkflow = readFile('.github/workflows/release.yml')
  const betaWorkflow = readFile('.github/workflows/beta-pack.yml')
  const localPackScript = readFile('scripts/pack-android-release.ps1')
  ...
  assert.match(buildGradle, /import java\.security\.KeyStore/)
  assert.match(buildGradle, /import java\.security\.MessageDigest/)
  assert.match(buildGradle, /import java\.util\.Base64/)
  assert.match(buildGradle, /def encodeSha1ToRedirectToken = \{/)
  assert.match(buildGradle, /def deriveOneDriveReleaseRedirectUri = \{/)
  assert.match(buildGradle, /def oneDriveReleaseRedirectUri = deriveOneDriveReleaseRedirectUri\(\)/)
  assert.match(buildGradle, /def oneDriveReleaseRedirectPath = decodeOneDriveRedirectPath\(oneDriveReleaseRedirectUri\)/)
  assert.doesNotMatch(buildGradle, /def oneDriveReleaseRedirectPath = oneDriveDebugRedirectPath/)
  assert.doesNotMatch(buildGradle, /ONEDRIVE_RELEASE_REDIRECT_URI/)
  assert.match(buildGradle, /generateReleaseOneDriveAuthConfig/)
  ...
  assert.match(releaseAuthConfigTemplate, /__ONEDRIVE_CLIENT_ID__/)
  assert.match(releaseAuthConfigTemplate, /__ONEDRIVE_REDIRECT_URI__/)
  assert.match(releaseAuthConfigTemplate, /"account_mode":\s*"SINGLE"/)
  assert.match(releaseAuthConfigTemplate, /"type":\s*"AzureADMultipleOrgs"/)
  assert.doesNotMatch(releaseWorkflow, /ONEDRIVE_RELEASE_REDIRECT_URI/)
  assert.doesNotMatch(betaWorkflow, /ONEDRIVE_RELEASE_REDIRECT_URI/)
  assert.doesNotMatch(localPackScript, /ONEDRIVE_RELEASE_REDIRECT_URI/)
