...
// 我的列表需要直接执行页面组件，验证菜单详情动作是否保持“独立页 / 外链”排他分流。
...
// 动态加载我的列表页面，并用最小桩组件捕获 ListMenu 回调与详情页导航调用。
...
test('我的列表页详情动作会排他地在应用内详情页和在线外链之间分流', () => {
  ...
  try {
    ...
    // 本地歌曲命中应用内详情后只能进入独立页，不能继续回退到在线外链。
    listMenu.props.onMusicSourceDetail({ musicInfo: localMusicInfo, listId: 'list_1' })
    ...
    listMenu.props.onMusicSourceDetail({ musicInfo: mediaLibraryMusicInfo, listId: 'list_1' })
    ...
    // 仅在线音源允许走外链回退，且此时不能误触发应用内详情页。
    listMenu.props.onMusicSourceDetail({ musicInfo: onlineMusicInfo, listId: 'list_1' })
