...
const mylistMusicListPath = path.resolve(__dirname, '../../src/screens/Home/Views/Mylist/MusicList/index.tsx')
...
const createMusicDetailModalTestRenderer = (component, componentProps = {}) => {
  const hookStates = []
  ...
  const rerender = () => {
    ...
    hookIndex = 0
    tree = resolveRenderedNode(component(componentProps, ref))
    return tree
  }
  ...
  return {
    ...
    findButtonsByText: text => flattenRenderedNodes(tree).filter(node => node.type === 'Button' && collectRenderedText(node) === text),
  }
}
...
const loadMusicDetailModalModule = ({
  ...
  mod.exports.__createTestRenderer = (props = {}) => createMusicDetailModalTestRenderer(mod.exports.default, props)
  return mod.exports
}

// 我的列表任务 4 需要直接执行页面组件，验证菜单详情动作是否保持“弹窗 / 外链”排他分流。
let currentMylistMusicListTestRuntime = null

const getMylistMusicListTestRuntime = () => {
  assert.ok(currentMylistMusicListTestRuntime, 'Mylist MusicList 测试运行时尚未初始化')
  return currentMylistMusicListTestRuntime
}

// 构造仅覆盖 useRef / useCallback 的极简页面渲染器，专门驱动 ListMenu 的详情动作回调。
const createMylistMusicListTestRenderer = component => {
  const hookStates = []
  const previousRuntime = currentMylistMusicListTestRuntime
  let hookIndex = 0
  let tree = null
  ...
  const rerender = () => {
    currentMylistMusicListTestRuntime = runtime
    hookIndex = 0
    tree = resolveRenderedNode(component({ onOpenMediaSourceManager: () => {} }))
    return tree
  }

  const cleanup = () => {
    currentMylistMusicListTestRuntime = previousRuntime
  }
...
// 动态加载我的列表页面，并用最小桩组件捕获 ListMenu 回调与详情弹窗 ref 调用。
const loadMylistMusicListModule = ({ listActionModule = loadListActionModule() } = {}) => {
  const source = fs.readFileSync(mylistMusicListPath, 'utf8')
  const transpiled = ts.transpileModule(source, {
    compilerOptions: {
      module: ts.ModuleKind.CommonJS,
      target: ts.ScriptTarget.ES2020,
    ...
    fileName: mylistMusicListPath,
  }).outputText

  const musicDetailShowCalls = []
  const artistRelatedSearchCalls = []
  const externalDetailCalls = []

  const createRefHostComponent = (type, createRefValue = () => ({})) => {
    return function MockComponent(props) {
      if (props.ref && typeof props.ref === 'object') props.ref.current = createRefValue(props)
      return getMylistMusicListTestRuntime().createHostNode(type, props)
    }
  }

  const mod = new Module(mylistMusicListPath, module)
  mod.filename = mylistMusicListPath
  mod.paths = Module._nodeModulePaths(path.dirname(mylistMusicListPath))
  mod.require = request => {
    switch (request) {
      case 'react':
        return {
          useCallback: fn => getMylistMusicListTestRuntime().useCallback(fn),
          useRef: value => getMylistMusicListTestRuntime().useRef(value),
        }
      case 'react/jsx-runtime':
        return {
          jsx: (type, props, key) => getMylistMusicListTestRuntime().createElement(type, props, key),
          jsxs: (type, props, key) => getMylistMusicListTestRuntime().createElement(type, props, key),
          Fragment: 'Fragment',
        }
      case 'react-native':
        return {
          View: createRefHostComponent('View'),
        }
      case '@/store/list/state':
        return {
          activeListId: 'list_1',
      ...
      case './ListMenu':
        return createRefHostComponent('ListMenu', () => ({
          show: () => {},
        }))
      case './listAction':
        return {
          ...listActionModule,
          handleShowMusicSourceDetail: async musicInfo => {
            externalDetailCalls.push(musicInfo)
          },
        }
      case './List':
        return createRefHostComponent('List', () => ({
          getSelectedList: () => [],
          scrollToInfo: () => {},
          scrollToTop: () => {},
          selectAll: () => {},
      ...
      case '@/components/MusicAddModal':
        return createRefHostComponent('ListMusicAdd', () => ({
          show: () => {},
        }))
      case '@/components/MusicMultiAddModal':
        return createRefHostComponent('ListMusicMultiAdd', () => ({
          show: () => {},
        }))
      case '@/utils/tools':
        return {
      ...
      case './ActiveList':
        return createRefHostComponent('ActiveList', () => ({
          setVisibleBar: () => {},
        }))
      case './MultipleModeBar':
        return createRefHostComponent('MultipleModeBar', () => ({
          exitSelectMode: () => {},
          setIsSelectAll: () => {},
          setSwitchMode: () => {},
          setVisibleBar: () => {},
      ...
      case './ListSearchBar':
        return createRefHostComponent('ListSearchBar', () => ({
          hide: () => {},
          show: () => {},
        }))
      case './ListMusicSearch':
        return createRefHostComponent('ListMusicSearch', () => ({
          hide: () => {},
          search: () => {},
          showArtistRelatedSongs: (artist, height) => {
            artistRelatedSearchCalls.push({ artist, height })
          },
        }))
      case './MusicPositionModal':
        return createRefHostComponent('MusicPositionModal', () => ({
          show: () => {},
        }))
      case '@/components/MetadataEditModal':
        return createRefHostComponent('MetadataEditModal', () => ({
          show: () => {},
        }))
      case './MusicToggleModal':
        return createRefHostComponent('MusicToggleModal', () => ({
          show: () => {},
        }))
      case '@/components/MusicDetailModal':
        return createRefHostComponent('MusicDetailModal', () => ({
          show: musicInfo => {
            musicDetailShowCalls.push(musicInfo)
          },
        }))
      default:
        throw new Error(`Unexpected dependency: ${request}`)
  ...
  mod._compile(transpiled, mylistMusicListPath)
  mod.exports.__createTestRenderer = () => createMylistMusicListTestRenderer(mod.exports.default)
  mod.exports.getMusicDetailShowCalls = () => [...musicDetailShowCalls]
  mod.exports.getArtistRelatedSearchCalls = () => [...artistRelatedSearchCalls]
  mod.exports.getExternalDetailCalls = () => [...externalDetailCalls]
  mod.exports.resetDetailRouteCalls = () => {
    musicDetailShowCalls.length = 0
    artistRelatedSearchCalls.length = 0
    externalDetailCalls.length = 0
  }
  return mod.exports
}
...
test('我的列表页详情动作会排他地在应用内弹窗和在线外链之间分流', () => {
  const musicListModule = loadMylistMusicListModule({
    listActionModule: loadListActionModule(),
  })
  const renderer = musicListModule.__createTestRenderer()
  ...
  try {
    const listMenu = renderer.findAllByType('ListMenu')[0]
    assert.ok(listMenu, '缺少 ListMenu 挂载结果，无法驱动详情菜单行为测试')
    assert.equal(typeof listMenu.props.onMusicSourceDetail, 'function')

    // 本地歌曲命中应用内详情后只能弹窗，不能继续回退到在线外链。
    listMenu.props.onMusicSourceDetail({ musicInfo: localMusicInfo })
    assert.deepEqual(musicListModule.getMusicDetailShowCalls(), [localMusicInfo])
    assert.deepEqual(musicListModule.getExternalDetailCalls(), [])

    musicListModule.resetDetailRouteCalls()

    // 媒体库歌曲与本地歌曲同属内部详情目标，也必须保持排他分流。
    listMenu.props.onMusicSourceDetail({ musicInfo: mediaLibraryMusicInfo })
    assert.deepEqual(musicListModule.getMusicDetailShowCalls(), [mediaLibraryMusicInfo])
    assert.deepEqual(musicListModule.getExternalDetailCalls(), [])

    musicListModule.resetDetailRouteCalls()

    // 仅在线音源允许走外链回退，且此时不能误触发应用内详情弹窗。
    listMenu.props.onMusicSourceDetail({ musicInfo: onlineMusicInfo })
    assert.deepEqual(musicListModule.getMusicDetailShowCalls(), [])
    assert.deepEqual(musicListModule.getExternalDetailCalls(), [onlineMusicInfo])
  } finally {
    renderer.cleanup()
  }
})
...
test('任务 3 详情弹窗歌手有值时会先关闭弹窗再触发 onPressArtist', () => {
  const pressCalls = []
  const modalModule = loadMusicDetailModalModule({
    translations: {
      music_detail_title: '歌曲详情',
      music_detail_name: '歌名',
      music_detail_artist: '歌手',
      music_detail_section_basic: '基础信息',
      music_detail_copy_name: '复制歌名',
      music_detail_copy_name_with_artist: '复制歌手 - 歌名',
      music_detail_copy_full: '复制完整信息',
      music_detail_copy_path: '复制路径',
      copy_name_tip: '已复制',
    },
  })
  const renderer = modalModule.__createTestRenderer({
    onPressArtist: payload => pressCalls.push(payload),
  })
  const musicInfo = {
    id: 'song_modal_artist_1',
    name: '海阔天空',
    singer: 'Beyond',
    source: 'local',
    interval: '04:13',
  ...
  try {
    ...
    const artistButtons = renderer.findButtonsByText('歌手：Beyond')
    assert.ok(artistButtons.length >= 1, '缺少可点击的歌手节点')

    artistButtons[0].props.onPress()
    assert.deepEqual(renderer.getDialogVisibilityCalls(), [true, false])
    assert.deepEqual(pressCalls, [{ artist: 'Beyond', musicInfo }])
  } finally {
    renderer.cleanup()
  }
})

test('任务 3 详情弹窗摘要区与基础信息区的歌手入口保持一致可点击', () => {
  const modalModule = loadMusicDetailModalModule({
    translations: {
      music_detail_title: '歌曲详情',
      music_detail_name: '歌名',
      music_detail_artist: '歌手',
      music_detail_album: '专辑',
      music_detail_interval: '时长',
      music_detail_source: '来源',
      music_detail_section_basic: '基础信息',
      music_detail_copy_name: '复制歌名',
      music_detail_copy_name_with_artist: '复制歌手 - 歌名',
      music_detail_copy_full: '复制完整信息',
  ...
  try {
    ...
    // 规格要求摘要区与基础信息区行为一致，因此这里锁定两个“歌手：Beyond”入口都应存在且都可点击。
    const artistButtons = renderer.findButtonsByText('歌手：Beyond')
    assert.equal(artistButtons.length, 2)
...
test('任务 3 我的列表页会把详情弹窗歌手点击转接到歌手相关歌曲浮层', () => {
  const musicListModule = loadMylistMusicListModule({
    listActionModule: loadListActionModule(),
  })
  const renderer = musicListModule.__createTestRenderer()
  ...
  try {
    const layoutView = renderer.findAllByType('View').find(node => typeof node.props.onLayout == 'function')
    assert.ok(layoutView, '缺少带 onLayout 的列表容器，无法注入高度')
    layoutView.props.onLayout({ nativeEvent: { layout: { height: 480 } } })

    const modal = renderer.findAllByType('MusicDetailModal')[0]
    assert.ok(modal, '缺少 MusicDetailModal 宿主节点')
    assert.equal(typeof modal.props.onPressArtist, 'function')

    modal.props.onPressArtist({ artist: 'Beyond', musicInfo: info })
    assert.deepEqual(musicListModule.getArtistRelatedSearchCalls(), [{ artist: 'Beyond', height: 480 }])
  } finally {
    renderer.cleanup()
  }
})
...
test('任务 3 歌手相关歌曲入口新增实现补齐中文注释', () => {
  const modalFile = readFile('src/components/MusicDetailModal/index.tsx')
  const indexFile = readFile('src/screens/Home/Views/Mylist/MusicList/index.tsx')

  assert.match(modalFile, /\/\/.*歌手可点击/)
  assert.match(modalFile, /\/\/.*关闭详情弹窗/)
  assert.match(indexFile, /\/\/.*歌手点击/)
  assert.match(indexFile, /\/\/.*当前列表内/)
})
