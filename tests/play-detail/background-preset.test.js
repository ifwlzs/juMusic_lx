...
test('page content rewires the emby variant to a mobile-safe shared background runtime instead of the old hard-band renderer', () => {
  ...
  assert.match(backgroundConfigFile, /vignetteOverlayColor:/)
  assert.match(backgroundConfigFile, /vignetteTransparentColor:/)
  assert.doesNotMatch(backgroundConfigFile, /brightnessOverlayColor: imageBrightnessDelta >= 0 \? '#ffffff' : '#000000'/)
  assert.doesNotMatch(backgroundConfigFile, /PlayDetailBackgroundVignetteBand/)
  assert.doesNotMatch(backgroundConfigFile, /PlayDetailBackgroundLinearVignetteSlice/)
  assert.doesNotMatch(backgroundConfigFile, /resolveVignetteBands =/)
  assert.doesNotMatch(backgroundConfigFile, /resolveLinearVignetteSlices =/)

  assert.match(backgroundLayerFile, /resolvedConfig\.blurLayers\.map/)
  assert.match(backgroundLayerFile, /import LinearGradient from 'react-native-linear-gradient'/)
  assert.match(backgroundLayerFile, /<LinearGradient/)
  ...
  assert.doesNotMatch(backgroundLayerFile, /resolvedConfig\.linearVignetteSlices\.map/)
  assert.doesNotMatch(backgroundLayerFile, /resolvedConfig\.vignetteBands\.flatMap/)
  assert.doesNotMatch(backgroundLayerFile, /renderVignetteBandStack/)
})
